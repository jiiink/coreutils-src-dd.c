...
alloc_ibuf(void)
{
    ...
    size_t buffer_size = input_blocksize + (extra_byte_for_swab ? 1 : 0);
...
print_xfer_stats(xtime_t progress_time)
{
    ...
    if (start_time < now) {
        ...
        size_t offset = bytes_per_second - bpsbuf;
        if (offset < bpsbufsize - sizeof(slash_s)) {
            strcpy(bpsbuf + offset + strlen(bytes_per_second), slash_s);
        }
    } else {
...
install_signal_handlers(void)
{
    ...
    if (sigaction(SIGINT, NULL, &old_act) == 0 && old_act.sa_handler != SIG_IGN)
  ...
  if (len > 0)
    {
      ...
      *pending = c_pending % IO_BUFSIZE;
      len = (c_pending > *pending) ? (c_pending - *pending) : 0;
    }
  else if (len == 0)
    {
      len = *pending;
               ...
               bool exclusive, char const *error_msgid)
{
  ...
  while (current && *current)
    {
      char const *comma_pos = strchr (current, ',');
      struct symbol_value const *entry = table;

      while (entry->symbol[0])
        {
          if (operand_matches (current, entry->symbol, ',') && entry->value)
            break;
          entry++;
        }
      ...
      if (!entry->symbol[0])
        {
          idx_t slen = comma_pos ? comma_pos - current : strlen (current);
...
{
  if (!str || !invalid) {
    if (invalid) *invalid = LONGINT_INVALID;
    return 0;
  }
  ...
  if (handle_byte_suffix(&e, &suffix, str)) {
    /* Suffix handling completed */
  }

  if ((e & ~LONGINT_OVERFLOW) == LONGINT_INVALID_SUFFIX_CHAR
      && *suffix == 'x')
    {
      result = handle_multiplication_suffix(str, suffix, n, e, &e);
    }
...
static bool handle_byte_suffix(strtol_error *e, char **suffix, char const *str)
{
  if ((*e & ~LONGINT_OVERFLOW) == LONGINT_INVALID_SUFFIX_CHAR
...
static intmax_t handle_multiplication_suffix(char const *str, char *suffix, 
                                           uintmax_t n, strtol_error orig_e, 
                                           strtol_error *e)
{
  ...
  if (ckd_mul (&result, n, o) || (result != 0 && ((orig_e | f) & LONGINT_OVERFLOW)))
    {
      *e = LONGINT_OVERFLOW;
      return INTMAX_MAX;
    }
...
static bool
validate_blocksize_parameter(const char *name, intmax_t n, intmax_t n_min, intmax_t n_max, 
                            idx_t *converted_idx, strtol_error *invalid)
{
    if (n < n_min)
...
static void
handle_blocksize_operand(const char *name, const char *val, intmax_t n, bool has_B)
{
    ...
    if (!validate_blocksize_parameter(name, n, n_min, n_max, converted_idx, &invalid)) {
...
static void
process_byte_flags(intmax_t skip, bool skip_B, intmax_t seek, bool seek_B, 
...
static void
validate_conversion_flags(void)
{
...
scanargs(int argc, char *const *argv)
{
    ...
    for (int i = optind; i < argc; i++) {
        ...
        if (operand_is(name, "if"))
            ...
            handle_blocksize_operand(name, val, n, has_B);
        }
    }
...
apply_translations (void)
{
  ...
  if (conversions_mask & C_UCASE)
    {
      for (i = 0; i < 256; i++)
        trans_table[i] = (unsigned char)toupper((unsigned char)trans_table[i]);
      translation_needed = true;
    }
  else if (conversions_mask & C_LCASE)
    {
      for (i = 0; i < 256; i++)
        trans_table[i] = (unsigned char)tolower((unsigned char)trans_table[i]);
      translation_needed = true;
    }
...
{
  if (*nread == 0 || buf == NULL || nread == NULL || saved_byte == NULL)
    return buf;

  int prev_saved = *saved_byte;
  int is_odd_read = (*nread & 1) != 0;
  int has_saved_byte = (prev_saved >= 0);

  if (has_saved_byte != is_odd_read)
    {
      --*nread;
...
{
    if (input_offset >= 0 && input_offset > INTMAX_MAX - offset)
        input_offset = -1;
      ...
      idx_t *bytes)
{
  ...
  if (can_seek(fdesc, records, blocksize, *bytes, &offset)) {
    return handle_seek_success(fdesc, file, records, blocksize, offset, bytes);
  }
  
  return handle_seek_failure(fdesc, file, records, blocksize, bytes);
}

static bool
can_seek(int fdesc, intmax_t records, idx_t blocksize, idx_t bytes, off_t *offset)
{
  return !ckd_mul(offset, records, blocksize) &&
         !ckd_add(offset, *offset, bytes) &&
         lseek(fdesc, *offset, SEEK_CUR) >= 0;
}

static intmax_t
handle_seek_success(int fdesc, char const *file, intmax_t records, 
                   idx_t blocksize, off_t offset, idx_t *bytes)
{
  if (fdesc == STDIN_FILENO) {
    return handle_stdin_seek(file, records, blocksize, offset);
  } else {
    *bytes = 0;
    return 0;
  }
...
static intmax_t
handle_stdin_seek(char const *file, intmax_t records, idx_t blocksize, off_t offset)
{
  struct stat st;
...
static bool
is_beyond_eof(struct stat *st, off_t offset)
{
  return usable_st_size(st) && 
         st->st_size > 0 && 
         input_offset >= 0 &&
...
static intmax_t
handle_seek_failure(int fdesc, char const *file, intmax_t records, 
                    idx_t blocksize, idx_t *bytes)
{
  int lseek_errno = errno;
  
  if (lseek(fdesc, 0, SEEK_END) >= 0) {
    handle_seekable_file_error(fdesc, file, lseek_errno);
  }
  
  return skip_by_reading(fdesc, file, records, blocksize, bytes, lseek_errno);
}

static void
handle_seekable_file_error(int fdesc, char const *file, int lseek_errno)
{
  if (!lseek_errno) {
...
{
  char *buf = get_buffer(fdesc);
  
  while (records > 0 || *bytes > 0) {
    idx_t read_size = records > 0 ? blocksize : *bytes;
...
static void
handle_read_error(int fdesc, char const *file, int lseek_errno)
{
  if (fdesc == STDIN_FILENO) {
  ...
  while (nread > 0)
    {
      ...
      idx_t nfree = (nread < available_space) ? nread : available_space;
...
dd_copy (void)
{
  ...
  exit_status = handle_input_skip();
  if (exit_status != EXIT_SUCCESS)
    return exit_status;

  exit_status = handle_output_seek();
  if (exit_status != EXIT_SUCCESS)
    return exit_status;
  ...
  while (should_continue_copying())
    {
      ...
      if (nread > 0)
        ...
        {
          exit_status = handle_read_error(&partread);
          if (exit_status == EXIT_FAILURE && !(conversions_mask & C_NOERROR))
            break;
          if (conversions_mask & C_NOERROR && exit_status != EXIT_FAILURE)
            {
      ...
      n_bytes_read = nread;
      update_block_counters(&n_bytes_read, &partread);

      if (ibuf == obuf)
        {
          exit_status = write_direct_buffer(n_bytes_read);
          if (exit_status != EXIT_SUCCESS)
            return exit_status;
          continue;
        }

      exit_status = process_buffer(&bufstart, &n_bytes_read, &saved_byte);
      if (exit_status != EXIT_SUCCESS)
        return exit_status;
    }

  exit_status = finalize_output(saved_byte);
  return exit_status;
}

static int
handle_input_skip(void)
{
  if (skip_records == 0 && skip_bytes == 0)
    return EXIT_SUCCESS;

  intmax_t us_bytes;
  bool us_bytes_overflow = (ckd_mul(&us_bytes, skip_records, input_blocksize) ||
                           ckd_add(&us_bytes, skip_bytes, us_bytes));
  off_t input_offset0 = input_offset;
  intmax_t us_blocks = skip(STDIN_FILENO, input_file, skip_records, 
...
static int
handle_output_seek(void)
{
  if (seek_records == 0 && seek_bytes == 0)
    return EXIT_SUCCESS;

  idx_t bytes = seek_bytes;
  intmax_t write_records = skip(STDOUT_FILENO, output_file, seek_records, 
...
static bool
should_continue_copying(void)
{
  return true;
}

static void
update_progress_if_needed(void)
{
...
static ssize_t
read_input_block(void)
{
  if (r_partial + r_full >= max_records)
    return iread_fnc(STDIN_FILENO, ibuf, max_bytes);
  else
    return iread_fnc(STDIN_FILENO, ibuf, input_blocksize);
}
...
static int
handle_read_error(idx_t *partread)
{
...
static void
update_block_counters(idx_t *n_bytes_read, idx_t *partread)
{
...
static int
write_direct_buffer(idx_t n_bytes_read)
{
...
static int
process_buffer(char **bufstart, idx_t *n_bytes_read, int *saved_byte)
{
...
static int
finalize_output(int saved_byte)
{
  if (0 <= saved_byte)
    {
          ...
          if (0 <= output_offset && stdout_stat.st_size < output_offset)
            {
              if (iftruncate(STDOUT_FILENO, output_offset) != 0)
                {
                  diagnose(errno, _("failed to truncate to %jd bytes in output file %s"),
