...
extern bool close_stdout_required;
...
static void diagnose(int errnum, char const *fmt, ...) {
    va_list ap;
    if (progress_len > 0) {
        fputc('\n', stderr);
        progress_len = 0;
...
void usage(int status) {
    if (status != EXIT_SUCCESS) {
        ...
        const char *basic_usage = _("Usage: %s [OPERAND]...\n  or:  %s OPTION\n");
        const char *operands_desc = _("\
...
\n\
  ...
  fsync     likewise, but also write metadata\n");
        ...
        const char *flags_desc = _("\nEach FLAG symbol may be:\n\n  append    append mode (makes sense only for output; conv=notrunc suggested)\n");
        const char *signals_desc = _("\nSending a %s signal to a running 'dd' process makes it\nprint I/O statistics to standard error and then resume copying.\n\nOptions are:\n\n");

        printf(basic_usage, program_name, program_name);
        fputs(operands_desc, stdout);
        ...
        printf(signals_desc, SIGINFO == SIGUSR1 ? "USR1" : "INFO");
...
static void alloc_ibuf(void) {
    ...
    if (ibuf == NULL) {
        ...
        error(EXIT_FAILURE, 0,
              ...
              input_blocksize,
              human_readable(input_blocksize, hbuf, human_opts | human_base_1024, 1, 1));
    }
}
...
static void alloc_obuf(void) {
    ...
    bool is_two_bufs = conversions_mask & C_TWOBUFS;
    ...
    if (!obuf) {
        ...
        error(EXIT_FAILURE, 0,
              ...
              output_blocksize,
              human_readable(output_blocksize, hbuf, human_opts | human_base_1024, 1, 1));
    }
}
...
static void translate_charset(const char *new_trans, const char *old_trans_table, char *new_trans_table) {
    if (!new_trans || !old_trans_table || !new_trans_table) {
        return;
    }
    
    for (size_t i = 0; i < CHARSET_SIZE; i++) {
        unsigned char old_index = (unsigned char)old_trans_table[i];
        new_trans_table[i] = new_trans[old_index];
    }
...
static void print_xfer_stats(xtime_t progress_time) {
    ...
    const char *iec = human_readable(w_bytes, hbuf[1], human_opts | human_base_1024, 1, 1);
    char *bpsbuf = hbuf[2];
    ...
    if (start_time < now) {
        ...
        strncat(bpsbuf, slash_s, bpsbufsize - strlen(bpsbuf) - 1);
    } else {
        delta_s = 0;
    ...
    char delta_s_buf[24];
    snprintf(delta_s_buf, sizeof delta_s_buf, progress_time ? "%.0f s" : "%g s", delta_s);

    int stats_len;
    if (abbreviation_lacks_prefix(si)) {
        stats_len = fprintf(stderr, ngettext("%jd byte copied, %s, %s", "%jd bytes copied, %s, %s", select_plural(w_bytes)),
                            w_bytes, delta_s_buf, bytes_per_second);
    } else if (abbreviation_lacks_prefix(iec)) {
        stats_len = fprintf(stderr, _("%jd bytes (%s) copied, %s, %s"), w_bytes, si, delta_s_buf, bytes_per_second);
    } else {
        stats_len = fprintf(stderr, _("%jd bytes (%s, %s) copied, %s, %s"), w_bytes, si, iec, delta_s_buf, bytes_per_second);
    }
...
static void print_stats(void) {
    if (status_level == STATUS_NONE || progress_len <= 0) {
        if (progress_len > 0) {
            fputc('\n', stderr);
            progress_len = 0;
        }
    ...
    fprintf(stderr, _("%" PRIdMAX "+%" PRIdMAX " records in\n%" PRIdMAX "+%" PRIdMAX " records out\n"),
            r_full, r_partial, w_full, w_partial);

    if (r_truncate != 0) {
        fprintf(stderr, ngettext("%" PRIdMAX " truncated record\n",
...
static void siginfo_handler(int sig) {
  static int info_signal_count = 0; 
  if (!(sigaction(SIGCHLD, NULL, NULL) == 0 && SA_NOCLDSTOP)) {
    struct sigaction sa;
...
static void handle_error(const char *message, const char *file) {
  error(EXIT_FAILURE, errno, message, quoteaf(file));
}
...
static void cleanup(void) {
  ...
  if (iclose(STDIN_FILENO) != 0) {
    handle_error(_("closing input file %s"), input_file);
  }

  if (iclose(STDOUT_FILENO) != 0) {
    handle_error(_("closing output file %s"), output_file);
  }
}
...
static void process_signals(void) {
    while (true) {
        ...
        if (!interrupt_signal && info_signal_count == 0) {
        ...
        int interrupt = interrupt_signal;
        sigprocmask(SIG_SETMASK, &oldset, nullptr);
        ...
        if (interrupt) {
            ...
            print_stats();
            raise(interrupt);
            break;  // Exit loop after handling interrupt
        }
...
static volatile sig_atomic_t signals_processed = false;
...
void process_signals(void) {
  if (signals_processed) {
    ...
    signals_processed = false; // Reset for next time
  }
}
...
#define IO_BUFSIZE 8192  // Define appropriate buffer size as per your application's needs
...
static off_t cache_round(int fd, off_t len) {
    ...
    if (len != 0) {
        intmax_t c_pending;
        if (*pending > INTMAX_MAX - len) {
            c_pending = INTMAX_MAX;
        } else {
            c_pending = *pending + len;
        }
...
off_t cache_round(int fd, off_t len);
...
static bool invalidate_cache(int fd, off_t len) {
    ...
    int adv_ret = posix_fadvise(fd, offset, clen, POSIX_FADV_DONTNEED);
    return adv_ret == 0;
#else
    errno = ENOTSUP;
    return false;
...
static ssize_t iread(int fd, char *buf, idx_t size) {
    ...
    if (nread > 0 && warn_partial_read && 0 < prev_nread && prev_nread < size) {
...
static ssize_t iread_fullblock(int fd, char *buf, idx_t size) {
    ...
    while (size > 0) {
        ...
        if (ncurr <= 0) {
            if (ncurr < 0 && errno == EINTR) {
                continue;
            }
            return (ncurr < 0) ? -1 : nread;
        }
        nread += ncurr;
        buf += ncurr;
        size -= ncurr;
...
static idx_t iwrite(int fd, char const *buf, idx_t size) {
    ...
    if ((output_flags & O_DIRECT) && size < output_blocksize) {
        ...
        if (fcntl(STDOUT_FILENO, F_SETFL, old_flags & ~O_DIRECT) != 0 && status_level != STATUS_NONE)
...
static int ifd_reopen(int desired_fd, const char *file, int flag, mode_t mode) {
    int ret;
...
static int parse_symbols(const char *str, const struct symbol_value *table, bool exclusive, const char *error_msgid) {
    ...
    while (str && *str) {
        ...
        for (const struct symbol_value *entry = table; entry->symbol[0]; ++entry) {
...
strtol_error xstrtoumax(const char *str, char **endptr, int base,
                        uintmax_t *result, const char *valid_suffixes);

int ckd_mul(intmax_t *result, uintmax_t a, intmax_t b);

void diagnose(int code, const char *fmt, const char *arg1, const char *arg2);

#define STRPREFIX(s, prefix) (strncmp(s, prefix, sizeof(prefix) - 1) == 0)

#define quote_n(n, string) string
...
static intmax_t parse_integer(const char *str, strtol_error *invalid) {
  ...
  if (e == LONGINT_INVALID_SUFFIX_CHAR && *suffix == 'B' && str < suffix &&
      suffix[-1] != 'B' && *(++suffix) == '\0') {
    e = LONGINT_OK;
  }
  ...
  if (e == LONGINT_INVALID_SUFFIX_CHAR && *suffix == 'x') {
    ...
    if (f != LONGINT_OK || ckd_mul(&n, n, o)) {
      *invalid = LONGINT_OVERFLOW;
      return INTMAX_MAX;
    }
    if (n == 0 && STRPREFIX(str, "0x")) {
      diagnose(0, "warning: %s is a zero multiplier; use %s if that is intended",
...
static void scanargs(int argc, char *const *argv) {
    idx_t blocksize = 0;
    intmax_t count = INTMAX_MAX, skip = 0, seek = 0;
    ...
    for (int i = optind; i < argc; i++) {
        ...
        if (operand_is(name, "if")) input_file = val;
        else if (operand_is(name, "of")) output_file = val;
        else if (operand_is(name, "conv")) conversions_mask |= parse_symbols(val, conversions, false, N_("invalid conversion"));
        else if (operand_is(name, "iflag")) input_flags |= parse_symbols(val, flags, false, N_("invalid input flag"));
        else if (operand_is(name, "oflag")) output_flags |= parse_symbols(val, flags, false, N_("invalid output flag"));
        else if (operand_is(name, "status")) status_level = parse_symbols(val, statuses, true, N_("invalid status level"));
        else {
            strtol_error invalid = LONGINT_OK;
            intmax_t n = parse_integer(val, &invalid);
            if (n < 0) invalid = LONGINT_INVALID;
            if (invalid != LONGINT_OK) error(EXIT_FAILURE, invalid == LONGINT_OVERFLOW ? EOVERFLOW : 0, "%s: %s", _("invalid number"), quoteaf(val));
            bool has_B = !!strchr(val, 'B');
            idx_t *converted_idx = NULL;

            idx_t max_blocksize = MIN(IDX_MAX - 1, MIN(SSIZE_MAX, OFF_T_MAX));
            if (operand_is(name, "ibs")) converted_idx = &input_blocksize;
            else if (operand_is(name, "obs")) converted_idx = &output_blocksize;
            else if (operand_is(name, "bs")) converted_idx = &blocksize;
            else if (operand_is(name, "cbs")) converted_idx = &conversion_blocksize;
            else if (operand_is(name, "skip") || operand_is(name, "iseek")) { skip = n; skip_B = has_B; }
            else if (operand_is(name + (*name == 'o'), "seek")) { seek = n; seek_B = has_B; }
            else if (operand_is(name, "count")) { count = n; count_B = has_B; }
            else {
                diagnose(0, _("unrecognized operand %s"), quoteaf(name));
                usage(EXIT_FAILURE);
            }

            if (converted_idx && n > 0 && n <= max_blocksize) *converted_idx = n;
        }
    }

    if (blocksize) input_blocksize = output_blocksize = blocksize;
    else conversions_mask |= C_TWOBUFS;

    if (!input_blocksize) input_blocksize = DEFAULT_BLOCKSIZE;
    if (!output_blocksize) output_blocksize = DEFAULT_BLOCKSIZE;
    if (!conversion_blocksize) conversions_mask &= ~(C_BLOCK | C_UNBLOCK);

    if (input_flags & (O_DSYNC | O_SYNC)) input_flags |= O_RSYNC;
    if (output_flags & O_FULLBLOCK) {
        diagnose(0, "%s: %s", _("invalid output flag"), quote("fullblock"));
        usage(EXIT_FAILURE);
    }

    if (skip_B) input_flags |= O_SKIP_BYTES;
    if (input_flags & O_SKIP_BYTES && skip) {
        skip_records = skip / input_blocksize;
        skip_bytes = skip % input_blocksize;
    } else if (skip) skip_records = skip;

    if (count_B) input_flags |= O_COUNT_BYTES;
    if (input_flags & O_COUNT_BYTES && count != INTMAX_MAX) {
        max_records = count / input_blocksize;
        max_bytes = count % input_blocksize;
    } else if (count != INTMAX_MAX) max_records = count;

    if (seek_B) output_flags |= O_SEEK_BYTES;
    if (output_flags & O_SEEK_BYTES && seek) {
        seek_records = seek / output_blocksize;
        seek_bytes = seek % output_blocksize;
    } else if (seek) seek_records = seek;

    warn_partial_read = (!(conversions_mask & C_TWOBUFS) && !(input_flags & O_FULLBLOCK)
                         && (skip_records || (max_records && max_records < INTMAX_MAX) || (input_flags | output_flags) & O_DIRECT));

    iread_fnc = (input_flags & O_FULLBLOCK) ? iread_fullblock : iread;
    input_flags &= ~O_FULLBLOCK;

    if (multiple_bits_set(conversions_mask & (C_ASCII | C_EBCDIC | C_IBM))) error(EXIT_FAILURE, 0, _("cannot combine any two of {ascii,ebcdic,ibm}"));
    if (multiple_bits_set(conversions_mask & (C_BLOCK | C_UNBLOCK))) error(EXIT_FAILURE, 0, _("cannot combine block and unblock"));
    if (multiple_bits_set(conversions_mask & (C_LCASE | C_UCASE))) error(EXIT_FAILURE, 0, _("cannot combine lcase and ucase"));
    if (multiple_bits_set(conversions_mask & (C_EXCL | C_NOCREAT))) error(EXIT_FAILURE, 0, _("cannot combine excl and nocreat"));
    if (multiple_bits_set(input_flags & (O_DIRECT | O_NOCACHE)) || multiple_bits_set(output_flags & (O_DIRECT | O_NOCACHE)))
...
static void apply_translations(void) {
    ...
    if (conversions_mask & (C_UCASE | C_LCASE)) {
        for (int i = 0; i < 256; i++) {
            trans_table[i] = (conversions_mask & C_UCASE) ? toupper(trans_table[i]) : tolower(trans_table[i]);
        }
        translation_needed = true;
    }

    if (conversions_mask & (C_EBCDIC | C_IBM)) {
        const unsigned char *translation_table = (conversions_mask & C_EBCDIC) ? ascii_to_ebcdic : ascii_to_ibm;
        translate_charset(translation_table);
        newline_character = translation_table['\n'];
        space_character = translation_table[' '];
    }
}
...
static void translate_buffer(char *buf, idx_t nread) {
    ...
    for (idx_t i = 0; i < nread; ++i) {
        unsigned char current_char = (unsigned char)buf[i];
        buf[i] = trans_table[current_char];
    }
}
...
static bool safe_add(intmax_t *result, intmax_t a, intmax_t b) {
...
static void advance_input_offset(intmax_t offset) {
    if (input_offset >= 0) {
        intmax_t new_offset;
        if (!safe_add(&new_offset, input_offset, offset)) {
            input_offset = -1;
            errno = ERANGE;  // Set errno to indicate range error
        } else {
...
static intmax_t skip(int fdesc, const char *file, intmax_t records, idx_t blocksize, idx_t *bytes) {
    errno = 0;
    off_t offset;
    
    if (!ckd_mul(&offset, records, blocksize) && !ckd_add(&offset, offset, *bytes) &&
        ...
        if (fdesc == STDIN_FILENO) {
            ...
            if (usable_st_size(&st) && st.st_size > 0 && input_offset >= 0 && (st.st_size - input_offset) < offset) {
        ...
        if (lseek(fdesc, 0, SEEK_END) < 0 || (offset < 0 && !lseek_errno)) {
            if (!lseek_errno) lseek_errno = EOVERFLOW;
            diagnose(lseek_errno, gettext(fdesc == STDIN_FILENO ? N_("%s: cannot skip") : N_("%s: cannot seek")), quotef(file));
            quit(EXIT_FAILURE);
        }
    
        char *buf = fdesc == STDIN_FILENO ? alloc_ibuf(), ibuf : alloc_obuf(), obuf;
        
        while (records || *bytes) {
            ssize_t nread = iread_fnc(fdesc, buf, records ? blocksize : *bytes);
            if (nread < 0) {
                diagnose(fdesc == STDIN_FILENO ? errno : lseek_errno, fdesc == STDIN_FILENO ? _("error reading %s") : _("%s: cannot seek"), quoteaf(file));
                if (fdesc == STDIN_FILENO && conversions_mask & C_NOERROR) print_stats();
                quit(EXIT_FAILURE);
            }
            if (nread == 0) break;
            if (fdesc == STDIN_FILENO) advance_input_offset(nread);
            if (records != 0) records--; else *bytes = 0;
        }
    }
    return records;
}
...
bool advance_input_after_read_error(idx_t nbytes) {
    if (!input_seekable) {
    ...
    if (!advance_input_offset(nbytes)) {
    ...
    if ((lseek(STDIN_FILENO, diff, SEEK_CUR) >= 0) || (errno == 0 && diagnose(0, _("cannot work around kernel bug after all")))) {
        return true;
    }
...
static void set_fd_flags(int fd, int add_flags, char const *name) {
    ...
    if (add_flags) {
        ...
        if (old_flags < 0) {
            error(EXIT_FAILURE, errno, "getting flags for %s", name);
            return;
        }
        ...
        if (new_flags & (O_DIRECTORY | O_NOLINKS)) {
            struct stat st;
            if (fstat(fd, &st) != 0) {
                error(EXIT_FAILURE, errno, "fstat error on %s", name);
                return;
            }

            if ((new_flags & O_DIRECTORY) && !S_ISDIR(st.st_mode)) {
                error(EXIT_FAILURE, ENOTDIR, "not a directory: %s", name);
                return;
            }

            if ((new_flags & O_NOLINKS) && st.st_nlink > 1) {
                error(EXIT_FAILURE, EMLINK, "too many links: %s", name);
                return;
            }
        ...
        if (fcntl(fd, F_SETFL, new_flags) == -1) {
            error(EXIT_FAILURE, errno, "setting flags for %s", name);
        }
    }
}
...
static int dd_copy(void) {
    ...
    while (true) {
        ...
        nread = perform_read(&partread);
        if (nread < 0) {
            if (!handle_read_error(&exit_status, partread)) {
                break;
            }
        } else if (nread == 0) {
            handle_eof();
            ...
            handle_read_success(nread, &saved_byte);
        }
    }

    handle_final_swab(&saved_byte);
    handle_final_buffer();

    if (final_op_was_seek) {
        if (!handle_final_seek()) {
            return EXIT_FAILURE;
        }
    }
...
static bool handle_skip(void) {
    intmax_t us_bytes;
    bool us_bytes_overflow = (ckd_mul(&us_bytes, skip_records, input_blocksize) || ckd_add(&us_bytes, skip_bytes, us_bytes));
...
static bool check_status_progress(void) {
...
static ssize_t perform_read(idx_t* partread) {
    ssize_t nread;
...
static bool handle_read_error(int* exit_status, idx_t partread) {
...
static void handle_read_success(ssize_t nread, int* saved_byte) {
    ...
    if (n_bytes_read < input_blocksize) {
        ...
        if (conversions_mask & C_SYNC) {
            if (!(conversions_mask & C_NOERROR)) {
                memset(ibuf + n_bytes_read, (conversions_mask & (C_BLOCK | C_UNBLOCK)) ? ' ' : '\0', input_blocksize - n_bytes_read);
            }
    ...
    if (ibuf == obuf) {
        handle_direct_write(n_bytes_read);
    } else {
        handle_translations(n_bytes_read, saved_byte);
    }
}

static void handle_direct_write(idx_t n_bytes_read) {
    idx_t nwritten = iwrite(STDOUT_FILENO, obuf, n_bytes_read);
    w_bytes += nwritten;
    if (nwritten != n_bytes_read) {
        diagnose(errno, _("error writing %s"), quoteaf(output_file));
    }
    w_full += (n_bytes_read == input_blocksize);
    w_partial += (n_bytes_read != input_blocksize);
}

static void handle_translations(idx_t n_bytes_read, int* saved_byte) {
    char* bufstart;
...
static void handle_final_swab(int* saved_byte) {
    if (0 <= *saved_byte) {
...
static void handle_final_buffer(void) {
...
static bool handle_final_seek(void) {
    ...
    if (S_ISREG(stdout_stat.st_mode) || S_TYPEISSHM(&stdout_stat)) {
        ...
        if (0 <= output_offset && stdout_stat.st_size < output_offset) {
            if (iftruncate(STDOUT_FILENO, output_offset) != 0) {
                diagnose(errno, _("failed to truncate to %jd bytes in output file %s"), (intmax_t)output_offset, quoteaf(output_file));
                return false;
            }
        }
    }
...
static bool need_sync(void) {
    return conversions_mask & (C_FDATASYNC | C_FSYNC) && status_level == STATUS_PROGRESS && 0 <= reported_w_bytes && reported_w_bytes < w_bytes;
}
...
extern void bindtextdomain(const char *package, const char *dirname);
extern void textdomain(const char *package);
extern void maybe_close_stdout(void);
extern void parse_gnu_standard_options_only(int argc, char **argv, const char *program_name, const char *package, const char *version, int longindex, void (*usage_func)(int), const char *authors, const char *translation_domain);
extern void scanargs(int argc, char **argv);
extern void apply_translations(void);
extern void set_fd_flags(int fd, int flags, const char *file);
extern int ifd_reopen(int fd, const char *file, int flags, mode_t mode);
extern int ckd_mul(off_t *result, off_t a, off_t b);
extern int ckd_add(off_t *result, off_t a, off_t b);
extern void error(int status, int errnum, const char *format, ...);
extern int lseek(int fd, int offset, int whence);
extern void diagnose(int errnum, const char *format, ...);
extern int dd_copy(void);
extern int synchronize_output(void);
extern int gethrxtime(void);
extern int XTIME_PRECISION;
extern off_t input_offset, input_seek_errno;
extern int close_stdout_required, input_flags, output_flags, conversions_mask;
extern int seek_records, output_blocksize;
extern char *input_file, *output_file;
extern int page_size;
extern int i_nocache, o_nocache, i_nocache_eof, o_nocache_eof;
extern off_t seek_bytes, max_records, max_bytes;
extern char trans_table[256];
extern int main_exit(int status);
extern int iftruncate(int fd, off_t length);
extern bool S_TYPEISSHM(const struct stat *statbuf);
extern void finish_up(void);
extern void invalidate_cache(int fd, int arg);
...
int main(int argc, char **argv) {
    ...
    parse_gnu_standard_options_only(argc, argv, PROGRAM_NAME, PACKAGE, Version, 1, NULL, NULL, NULL);
    close_stdout_required = 0;

    for (int i = 0; i < 256; i++) trans_table[i] = i;
    ...
    if (!input_file) {
        ...
        if (ifd_reopen(STDIN_FILENO, input_file, O_RDONLY | input_flags, 0) < 0)
            error(EXIT_FAILURE, errno, _("failed to open %s"), input_file);
    }

    off_t offset = lseek(STDIN_FILENO, 0, SEEK_CUR);
    int input_seekable = (0 <= offset);
    ...
    if (!output_file) {
        ...
        int opts = output_flags | ((conversions_mask & 1 ? 0 : O_CREAT) | (conversions_mask & 2 ? O_EXCL : 0) | ((seek_records || (conversions_mask & 4)) ? 0 : O_TRUNC));

        off_t size;
        if ((ckd_mul(&size, seek_records, output_blocksize) || ckd_add(&size, seek_bytes, size)) && !(conversions_mask & 4))
            error(EXIT_FAILURE, 0, _("offset too large: cannot truncate to a length of seek=%jd (%td-byte) blocks"), seek_records, output_blocksize);

        if ((!seek_records || ifd_reopen(STDOUT_FILENO, output_file, O_RDWR | opts, perms) < 0) && ifd_reopen(STDOUT_FILENO, output_file, O_WRONLY | opts, perms) < 0)
            error(EXIT_FAILURE, errno, _("failed to open %s"), output_file);

        if (seek_records && !(conversions_mask & 4)) {
            if (iftruncate(STDOUT_FILENO, size) != 0) {
                int ftruncate_errno = errno;
                struct stat stdout_stat;
                if (ifstat(STDOUT_FILENO, &stdout_stat) != 0) {
                    diagnose(errno, _("cannot fstat %s"), output_file);
                    return EXIT_FAILURE;
                } else if (S_ISREG(stdout_stat.st_mode) || S_ISDIR(stdout_stat.st_mode) || S_TYPEISSHM(&stdout_stat)) {
                    diagnose(ftruncate_errno, _("failed to truncate to %jd bytes in output file %s"), size, output_file);
                    return EXIT_FAILURE;
                }
            }
        }
    ...
    int start_time = gethrxtime();
    int next_time = start_time + XTIME_PRECISION;

    int exit_status = dd_copy();

    int sync_status = synchronize_output();
    if (sync_status)
        exit_status = sync_status;

    if (!max_records && !max_bytes) {
        if (i_nocache && !invalidate_cache(STDIN_FILENO, 0)) {
            diagnose(errno, _("failed to discard cache for: %s"), input_file);
            exit_status = EXIT_FAILURE;
        }
        if (o_nocache && !invalidate_cache(STDOUT_FILENO, 0)) {
            diagnose(errno, _("failed to discard cache for: %s"), output_file);
            exit_status = EXIT_FAILURE;
        }
    } else {
